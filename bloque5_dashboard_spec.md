...
> **Estado de las medidas:** este documento refleja las versiones **finales** implementadas
> en el `.pbix`. Tres medidas (`GMV Comparable`, `Clientes Retenidos`, `Dias Sin Venta`) se
> ajustaron durante la construcción del dashboard respecto a su versión inicial; cada una
> lleva una nota de implementación explicando qué se cambió y por qué. El detalle del proceso
> de depuración está documentado en el `README.md`.
...
GMV Comparable =
-- NOTA DE IMPLEMENTACIÓN: la versión original usaba ALL(Dim_Store), que borra TODOS
-- los filtros de la dimensión tienda. Funcionaba para el KPI global, pero al desglosar
-- por formato en el gráfico de líneas mostraba el MISMO valor para los 4 formatos
-- (el ALL eliminaba también el filtro de formato de la leyenda). Se cambió a ALLEXCEPT
-- preservando format/country/region para que el desglose respete esos filtros y solo
-- se aplique la regla de elegibilidad de 13 meses.
CALCULATE(
    [GMV Neto],
    FILTER(
        ALLEXCEPT(Dim_Store, Dim_Store[format], Dim_Store[country], Dim_Store[region]),
...
Clientes Retenidos =
-- NOTA DE IMPLEMENTACIÓN: la versión original usaba DATEVALUE(CohorteSel & "-01"),
-- que falla con configuración regional en español (error "Failed to move the data
-- reader to the next row"). DATEVALUE interpreta el string según el formato de fecha
-- local. Se reemplazó por DATE() construyendo la fecha a partir de año y mes extraídos
-- del string, que es independiente del idioma/región.
VAR Offset = SELECTEDVALUE(Offsets[Mes Offset])
VAR CohorteSel = SELECTEDVALUE(Dim_Customer[Cohorte])
VAR AnioCohorte = VALUE(LEFT(CohorteSel, 4))
VAR MesCohorte = VALUE(MID(CohorteSel, 6, 2))
VAR MesObjetivo = EDATE(DATE(AnioCohorte, MesCohorte, 1), Offset)
RETURN
CALCULATE(
    DISTINCTCOUNT(Fact_Sales[customer_id]),
    Fact_Sales[loyalty_card] = TRUE(),
...
```dax
Dias Sin Venta =
-- NOTA DE IMPLEMENTACIÓN: la versión original usaba ALLEXCEPT sobre store_id/item_id,
-- pero devolvía 0 para todos los productos cuando la tabla mostraba solo los NOMBRES
-- (store_name, item_name) sin los IDs: el contexto no bajaba al grano tienda-producto
-- y la medida evaluaba sobre un conjunto más amplio, donde la última venta coincidía
-- con el fin de datos. Se reescribió forzando el grano DENTRO de la medida con
-- SUMMARIZE, lo que permite mostrar solo los nombres en la tabla sin exponer los IDs.
VAR FinDeDatos =
    CALCULATE(
...
RETURN
    MAXX(
        SUMMARIZE(Fact_Sales, Dim_Store[store_id], Dim_Product[item_id]),
        VAR UltimaVentaAqui =
            CALCULATE(
                MAX(Fact_Sales[transaction_date]),
                Fact_Sales[status] = "COMPLETED"
        ...
        RETURN
            IF(
                NOT ISBLANK(UltimaVentaAqui),
                DATEDIFF(UltimaVentaAqui, FinDeDatos, DAY)
            )
    )
